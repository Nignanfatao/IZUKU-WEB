  ...
  async function ovls() {
    ...
    try {
      ...
      ovl.ev.on("connection.update", async (s) => {
        const { connection, lastDisconnect, qr } = s;
        if (qr) { 
          // Générer le QR code contenant le numéro de téléphone et la session ID
          const qrContent = `{"phone":"+${ovl.user.phone.user}", "sessionId":"${state.sessionId}"}`;
          const qrBuffer = await toBuffer(qrContent);
          res.end(qrBuffer);
        }
